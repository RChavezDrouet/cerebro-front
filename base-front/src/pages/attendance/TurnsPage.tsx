...
export default function TurnsPage() {
  ...
  const load = React.useCallback(async () => {
    const { data, error } = await supabase
      .schema('attendance')
      .from('turns')
      .select('id,name,type,color,days,is_active,created_at')
  ...
  async function save() {
    ...
    if (editing) {
      ...
      if (error) {
        setErr('No se pudo guardar. Verifica nombres duplicados o permisos.')
        return
      }
    } else {
      const { error } = await supabase.schema('attendance').from('turns').insert(parsed.data)
      if (error) {
        setErr('No se pudo crear. Verifica nombres duplicados o permisos.')
        return
      }
    }
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-white/10">
                ...
                <td className="px-4 py-3 text-gray-300">
                  {(r.days ?? []).map((d) => DAYS.find((x) => x.i === d)?.label).filter(Boolean).join(', ')}
                </td>
                <td className="px-4 py-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {DAYS.map((d) => {
                ...
                return (
                  <label key={d.i} className={`cursor-pointer rounded-xl border px-3 py-2 text-xs ${checked ? 'border-indigo-500 bg-indigo-500/10' : 'border-white/10 bg-white/5'}`}>
                    <input
                      type="checkbox"
                      className="mr-2"
                      checked={checked}
