...
async function fetchBiometricDevices(tenantId: string) {
  const advanced = await supabase
    ...
    .select('id,serial_no,name,is_active,location_alias,display_order')
    .eq('tenant_id', tenantId)
    .eq('is_active', true)
  ...
  const basic = await supabase
    .schema(ATT_SCHEMA)
    .from('biometric_devices')
    .select('id,serial_no,name,is_active')
    .eq('tenant_id', tenantId)
    .eq('is_active', true)
    .order('name')

  if (basic.error) return []
  return (basic.data ?? []) as Array<{
    id: string
    serial_no: string
    name: string | null
    is_active: boolean
...
export default function EmployeeFormPage({ mode }: Props) {
  ...
  const bioOptions = (bioDevices.data ?? []).map((d) => {
    const alias = (d.location_alias ?? '').trim()
    const technicalName = (d.name ?? '').trim()
    const primary = alias || technicalName || d.serial_no
    const secondary = d.serial_no && primary !== d.serial_no ? ` · ${d.serial_no}` : ''
    return { value: d.id, label: `${primary}${secondary}` }
  })
