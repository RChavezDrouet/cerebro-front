...
async function resolveTenantIdForUser(
  ...
  if (jwtTenantId && isUuid(String(jwtTenantId))) {
    if (requestedTenantId && requestedTenantId !== String(jwtTenantId)) {
      throw new Error('El tenant solicitado no coincide con el tenant del usuario')
    }
    return String(jwtTenantId)
  }
  ...
  for (const [, rows] of byEmployeeDate) {
    ...
    if (inOutRows.length % 2 !== 0) {
      novelties.push({
        ...
        severity: 'high',
        description: `Cantidad impar de marcaciones IN y OUT (${inOutRows.length}) en el día.`,
      })
    }
