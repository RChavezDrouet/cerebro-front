...
export const useAttendance = (profile: UserProfile | null) => {
  ...
  const loadTodayPunches = useCallback(async () => {
    ...
    const selectWithVer = 'id,tenant_id,employee_id,punched_at,source,serial,status,verification,evidence,created_at'
    const selectNoVer  = 'id,tenant_id,employee_id,punched_at,source,serial,status,evidence,created_at'

    let res = await attendanceDb
      .from('punches')
      .select(selectWithVer)
    ...
    if (
      res.error &&
      String(res.error.message || '').toLowerCase().includes('verification') &&
      String(res.error.message || '').toLowerCase().includes('does not exist')
    ) {
      res = await attendanceDb
        .from('punches')
        .select(selectNoVer)
      .eq('tenant_id', profile.tenant_id)
      .eq('employee_id', profile.employee_id)
  ...
  const registerPunch = useCallback(
    async (
      ...
      // Fallback si tu BD no tiene columna 'verification'
      if (
        error &&
        String(error.message || '').toLowerCase().includes('verification') &&
        String(error.message || '').toLowerCase().includes('does not exist')
      ) {
        const { verification, ...payloadNoVer } = payload as any
        ;({ error } = await attendanceDb.from('punches').insert(payloadNoVer))
      }
