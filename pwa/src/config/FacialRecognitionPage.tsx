...
export const FacialRecognitionPage: React.FC = () => {
  ...
  const loadConfig = async () => {
    ...
    try {
      ...
      const { data } = await supabase
        ...
        .single();
      if (data) setConfig(data as FacialRecognitionConfig);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (config.id) {
        await supabase.schema('attendance').from('facial_recognition_config').update(payload).eq('id', config.id);
      } else {
        await supabase.schema('attendance').from('facial_recognition_config').insert(payload);
      }
