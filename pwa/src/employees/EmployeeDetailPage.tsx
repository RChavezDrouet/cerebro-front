...
function biometricDetailLabel(device: Pick<BiometricOptionRow, 'serial_no' | 'name' | 'location_alias' | 'location_details' | 'display_alias'>) {
...
async function fetchOrgHistory(tenantId: string, employeeId: string) {
  ...
  return (data ?? []) as Array<{ effective_from?: string | null; effective_to?: string | null; org_unit_id?: string | null }>
}
...
async function fetchShiftHistory(tenantId: string, employeeId: string) {
  ...
  return (data ?? []) as Array<{ effective_from?: string | null; effective_to?: string | null; shift_id?: string | null }>
}
...
export default function EmployeeDetailPage() {
  ...
  const emp = useQuery({ queryKey: ['employee', tenantId, id], enabled: !!tenantId && !!id, queryFn: () => fetchEmployee(tenantId!, id!) })
  const photo = useQuery({ queryKey: ['photo', emp.data?.facial_photo_url], enabled: !!emp.data?.facial_photo_url, queryFn: () => signedPhoto(emp.data?.facial_photo_url) })
  const orgAssignment = useQuery({ queryKey: ['org-assignment', tenantId, id], enabled: !!tenantId && !!id, queryFn: () => fetchEmployeeOrgAssignment(tenantId!, id!) })
  const shiftAssignment = useQuery({ queryKey: ['shift-assignment', tenantId, id], enabled: !!tenantId && !!id, queryFn: () => fetchEmployeeShiftAssignment(tenantId!, id!) })
  ...
  const shifts = useQuery({ queryKey: ['shift-lookup', tenantId], enabled: !!tenantId, queryFn: () => fetchShiftLookup(tenantId!) })
  const bioDevices = useQuery({ queryKey: ['biometric-device-config', tenantId], enabled: !!tenantId, queryFn: () => fetchBiometricDevices(tenantId!) })
  const accessInfo = useQuery({ queryKey: ['employee-access', tenantId, id, (emp.data as any)?.user_id], enabled: !!tenantId && !!id, queryFn: () => fetchEmployeeAccess(tenantId!, id!, (emp.data as any)?.user_id ?? null) })
  const orgHistory = useQuery({ queryKey: ['org-history', tenantId, id], enabled: !!tenantId && !!id, queryFn: () => fetchOrgHistory(tenantId!, id!) })
  const shiftHistory = useQuery({ queryKey: ['shift-history', tenantId, id], enabled: !!tenantId && !!id, queryFn: () => fetchShiftHistory(tenantId!, id!) })
  ...
  const modality = e.work_modality ?? e.work_mode ?? 'PRESENCIAL'
  ...
  const leaderUnit = (orgUnits.data ?? []).find((row) => row.id === (orgAssignment.data?.lead_org_unit_id ?? orgAssignment.data?.org_unit_id))
  const leaderLevelLabel = (orgLevels.data ?? []).find((row) => row.level_no === leaderUnit?.level_no)?.display_name ?? null
  ...
  const entryDevice = (bioDevices.data ?? []).find((row) => row.id === e.entry_biometric_id)
  const exitDevice = (bioDevices.data ?? []).find((row) => row.id === e.exit_biometric_id)
  const entryLocationLabel = entryDevice
    ? biometricDetailLabel(entryDevice as any)
    : (e.entry_biometric_location ?? e.entry_biometric_id ?? '—')
  const exitLocationLabel = exitDevice
    ? biometricDetailLabel(exitDevice as any)
    : (e.exit_biometric_location ?? e.exit_biometric_id ?? '—')
  ...
  const shiftRows = (shiftHistory.data ?? []).map((row) => ({
    effective_from: row.effective_from,
    effective_to: row.effective_to,
    label: row.shift_id ? ((shifts.data ?? []).find((shift) => shift.id === row.shift_id)?.name ?? 'Turno asignado') : 'Sin turno específico',
  }))
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-bold">{e.first_name} {e.last_name}</h1>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            <span className="text-sm text-white/60">Código: <span className="font-semibold text-white">{e.employee_code ?? e.employee_number ?? '—'}</span></span>
          </div>
        </div>
        <Link to={`/employees/${id}/edit`}><Button leftIcon={<Edit3 size={16} />}>Editar</Button></Link>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <Card title="Ficha" className="xl:col-span-2">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            <div className="card p-4"><div className="text-white/60">Departamento</div><div className="mt-2 font-semibold">{e.department_name ?? '—'}</div></div>
        ...
        <Card title="Fotografía" subtitle="Reconocimiento facial" actions={<Camera size={18} className="text-white/60" />}>
          {photo.data ? <img src={photo.data} className="w-full rounded-2xl border border-white/10" alt="Foto empleado" /> : <div className="text-sm text-white/60">Sin foto registrada.</div>}
        </Card>
      </div>
      ...
      <Card title="Modalidad de trabajo" actions={<Briefcase size={18} className="text-white/60" />}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div>
            ...
            <Badge tone={modality === 'REMOTO' ? 'info' : modality === 'MIXTO' ? 'neutral' : 'good'}>{modality}</Badge>
          </div>
          ...
          {modality === 'MIXTO' && (
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {presentialDays.length > 0 ? presentialDays.sort().map((d) => (
                  <span key={d} className="rounded-full px-2 py-0.5 bg-white/10 text-xs font-medium">{DAYS_LABEL[d] ?? d}</span>
            ...
            <div>
              ...
              {e.location_mode === 'UBICACION' && (
                <div className="mt-1 text-xs text-white/50 space-y-0.5">
                  <div>Entrada: {entryLocationLabel}</div>
                  <div>Salida: {exitLocationLabel}</div>
                </div>
              )}
            </div>
          )}
      ...
      <Card title="Turno y horario efectivo" actions={<Workflow size={18} className="text-white/60" />}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div className="card p-4">
            ...
            <div className="mt-2 font-semibold">{shiftInfo ? `${shiftInfo.name}${shiftInfo.code ? ` (${shiftInfo.code})` : ''}` : 'Sin sobrescritura individual'}</div>
          </div>
          <div className="card p-4">
