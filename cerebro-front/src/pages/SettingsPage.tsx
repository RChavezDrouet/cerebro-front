 ...
 * - Planes (cerebro.subscription_plans)
...
import ProductCatalogSettingsPanel from '../components/settings/ProductCatalogSettingsPanel'
...
const SECTIONS: Array<{ key: SectionKey; label: string; icon: any }> = [
  ...
  { key: 'catalog', label: 'Productos / Tarifas', icon: CreditCard },
...
type PlanPriceModel = 'fixed' | 'per_user'
...
const normalizePlanPriceModel = (value: unknown): PlanPriceModel => {
  const normalized = String(value || '').trim().toLowerCase()
  if (normalized === 'per_user' || normalized === 'usage') return 'per_user'
  return 'fixed'
}
...
const planPriceModelLabel = (value: unknown) => {
  const normalized = normalizePlanPriceModel(value)
  return normalized === 'per_user' ? 'Por usuario' : 'Fijo'
}
...
export default function SettingsPage() {
  ...
  const [planEditingCode, setPlanEditingCode] = useState<string | null>(null)
  ...
  const resetPlanEditor = () => {
    setPlanEditingCode(null)
    setPlanForm(emptyPlanForm())
  }

  const plansTable = () => supabase.schema('cerebro').from('subscription_plans')

  const mapPlanToForm = (pl: PlanRow): PlanFormState => ({
    code: String(pl.code || ''),
    name: String(pl.name || ''),
    description: String(pl.description || ''),
    price_model: normalizePlanPriceModel(pl.price_model),
    price: Number(pl.price || 0),
    is_active: Boolean(pl.is_active ?? true),
  })
  ...
  const loadPlans = async () => {
    ...
    try {
      const { data, error } = await plansTable()
        .select('code, name, description, price, price_model, is_active, created_at')
      ...
      toast.error(error?.message || 'No se pudo cargar catálogo de planes')
      setPlans([])
    } finally {
  ...
  const saveBrand = async () => {
    ...
    setBrandSaving(true)
    try {
      const { data, removedColumns, error } = await upsertSingletonRow('app_settings', {
      ...
      if (removedColumns?.length) {
        toast('Advertencia: faltan columnas en app_settings. Ejecuta las migraciones para guardar todos los campos.', {
          icon: '⚠️',
        })
      }
    } finally {
  ...
  const saveSmtp = async () => {
    ...
    try {
      ...
      if (smtpPassword?.trim()) {
        ...
        if (error) {
          toast('SMTP guardado, pero NO se pudo guardar el secreto (Edge Function smtp-settings no desplegada o sin permisos).', {
            icon: '⚠️',
          })
        } else {
  ...
  const togglePerm = async (role: RoleKey, permKey: string) => {
    ...
    const previousRolePerms = { ...(perms[role] || {}) }
    ...
    try {
      const { error } = await supabase.from('role_permissions').upsert({ role, permissions: nextRolePerms }, { onConflict: 'role' })

      if (error) {
        toast.error('No se pudo guardar permisos (role_permissions).')
        setPerms((prev) => ({ ...prev, [role]: previousRolePerms }))
      }
    } finally {
  ...
  const savePlan = async () => {
    ...
    try {
      const code = String(planForm.code || '').trim().toLowerCase()
      const name = String(planForm.name || '').trim()

      if (!code) return toast.error('Código de plan requerido')
      if (!name) return toast.error('Nombre de plan requerido')

      const payload = {
        code,
        name,
        description: String(planForm.description || '').trim() || null,
        price_model: normalizePlanPriceModel(planForm.price_model),
        price: Number(planForm.price || 0),
        is_active: Boolean(planForm.is_active),
      }

      const { error } = await plansTable().upsert(payload, { onConflict: 'code' })
      if (error) throw error

      toast.success(planEditingCode ? 'Plan actualizado' : 'Plan guardado')
      resetPlanEditor()
      await loadPlans()
    } catch (e: any) {
  ...
  const editPlan = (pl: PlanRow) => {
    setPlanEditingCode(pl.code)
    setPlanForm(mapPlanToForm(pl))
  }
  ...
  const createUser = async () => {
    ...
    try {
      const email = String(userCreate.email || '').trim().toLowerCase()
      const fullName = String(userCreate.full_name || '').trim()
      const password = String(userCreate.temp_password || '')
      const role = String(userCreate.role || 'assistant').trim()
      const rotationDays = Number(userCreate.rotation_days || 0)
      const forcePasswordChange = Boolean(userCreate.force_password_change)
      ...
      if (!password || password.length < 8) {
        toast.error('Password temporal (>=8) requerido')
        return
      }
      ...
      if (sessionError) {
        console.error('getSession error before admin-create-user:', sessionError)
        toast.error('No se pudo validar la sesión actual. Cierra sesión y vuelve a ingresar.')
        return
      }

      const accessToken = session?.access_token
      if (!accessToken) {
        toast.error('Sesión expirada o no disponible. Cierra sesión y vuelve a ingresar.')
        return
      }
      ...
      const { data, error } = await supabase.functions.invoke('admin-create-user', {
        body: {
          ...
          full_name: fullName,
          role,
          rotation_days: Number.isFinite(rotationDays) ? rotationDays : 0,
      ...
      if (error) {
        const f = formatSupabaseError(error)
        const rawMessage = (error as any)?.context?.json?.error || (error as any)?.message || ''
        const hint = rawMessage || f.hint || 'Revisa consola y logs.'
        toast.error(`Usuario: ${f.title}. ${hint}`)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-9">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-400">
                      Este mensaje lo consume Base cuando detecta que el tenant está <b>paused</b>. Es global (único).
                    </p>
                  </div>
                  <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setBrand((p: any) => ({ ...p, company_name: e.target.value }))}
                  />
                </div>
                <div>
                  ...
                  <input
                    className="input-field"
                    value={brand.company_ruc || ''}
                    onChange={(e) => setBrand((p: any) => ({ ...p, company_ruc: e.target.value.replace(/\D/g, '').slice(0, 13) }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setBrand((p: any) => ({ ...p, primary_color: e.target.value }))}
                    placeholder="#00e673"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setBrand((p: any) => ({ ...p, secondary_color: e.target.value }))}
                    placeholder="#00b3ff"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setBrand((p: any) => ({ ...p, accent_color: e.target.value }))}
                    placeholder="#7c3aed"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    className="input-field"
                    value={brand.login_message_title || ''}
                    onChange={(e) => setBrand((p: any) => ({ ...p, login_message_title: e.target.value }))}
                  />
                </div>
                <div>
                  ...
                  <input
                    className="input-field"
                    value={brand.login_message_body || ''}
                    onChange={(e) => setBrand((p: any) => ({ ...p, login_message_body: e.target.value }))}
                  />
                </div>
              </div>

              <div className="flex justify-end">
                <button disabled={brandSaving || !canSettingsEdit} onClick={saveBrand} className="btn-primary inline-flex items-center gap-2">
          ...
          {active === 'catalog' && <ProductCatalogSettingsPanel />}
            ...
            <div className="card p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    className="input-field"
                    value={userCreate.temp_password}
                    onChange={(e) => setUserCreate((p: any) => ({ ...p, temp_password: e.target.value }))}
                    placeholder="Genera uno fuerte"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    className="input-field"
                    value={userCreate.rotation_days}
                    onChange={(e) => setUserCreate((p: any) => ({ ...p, rotation_days: Number(e.target.value) }))}
                  />
                </div>
                <div className="flex items-end">
                  <label className="inline-flex items-center gap-2 text-sm text-slate-200">
                    <input
                      type="checkbox"
                      checked={!!userCreate.force_password_change}
                      onChange={(e) => setUserCreate((p: any) => ({ ...p, force_password_change: e.target.checked }))}
                    />
          ...
          {active === 'smtp' && (
            <div className="card p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    className="input-field"
                    value={smtp.host || ''}
                    onChange={(e) => setSmtp((p: any) => ({ ...p, host: e.target.value }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={smtp.has_secret ? '*** (configurado)' : 'Escriba para actualizar'}
                  />
              ...
              <div className="flex justify-end">
                <button disabled={smtpSaving || !canSettingsEdit} onClick={saveSmtp} className="btn-primary inline-flex items-center gap-2">
                  <Save className="w-4 h-4" />
                ...
                <div className="flex flex-col md:flex-row md:items-end gap-3">
                  <div className="flex-1">
                    ...
                    <div className="mt-3">
                      ...
                      <input className="input-field" value={smtpTestEmail} onChange={(e) => setSmtpTestEmail(e.target.value)} placeholder="tu-correo@dominio.com" />
                    </div>
                  </div>
                  <button disabled={smtpTesting} onClick={testSmtp} className="btn-secondary inline-flex items-center gap-2 justify-center md:w-[180px]">
              ...
              <div className="rounded-2xl border border-[rgba(148,163,184,0.12)] bg-white/5 p-4 text-sm text-slate-300">
                ...
                <p className="mt-1">
                  Para almacenar el password en Vault/Secrets, despliega la Edge Function <code>smtp-settings</code> y configura el Service Role como secreto del proyecto (ver{' '}
                  <b>docs/SUPABASE_SETUP.md</b>).
                </p>
              </div>
            </div>
          )}
          ...
          {active === 'billing' && (
            <div className="card p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    className="input-field"
                    value={billing.tax_percent ?? 0}
                    onChange={(e) => setBilling((p: any) => ({ ...p, tax_percent: Number(e.target.value) }))}
                  />
                </div>
                <div className="md:col-span-2">
              ...
              <div className="flex justify-end">
                <button disabled={billingSaving || !canSettingsEdit} onClick={saveBilling} className="btn-primary inline-flex items-center gap-2">
                  <Save className="w-4 h-4" />
          ...
          {active === 'kpi' && (
            <div className="card p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="number"
                    className="input-field"
                    value={kpi.expected_revenue_monthly ?? 0}
                    onChange={(e) => setKpi((p: any) => ({ ...p, expected_revenue_monthly: Number(e.target.value) }))}
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    className="input-field"
                    value={kpi.expected_new_clients_monthly ?? 0}
                    onChange={(e) => setKpi((p: any) => ({ ...p, expected_new_clients_monthly: Number(e.target.value) }))}
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    className="input-field"
                    value={kpi.green_change_pct ?? 0}
                    onChange={(e) => setKpi((p: any) => ({ ...p, green_change_pct: Number(e.target.value) }))}
                  />
                ...
                <div>
                  ...
                  <input
                    type="number"
                    className="input-field"
                    value={kpi.yellow_change_pct ?? -5}
                    onChange={(e) => setKpi((p: any) => ({ ...p, yellow_change_pct: Number(e.target.value) }))}
                  />
              ...
              <div className="flex justify-end">
                <button disabled={kpiSaving || !canSettingsEdit} onClick={saveKpi} className="btn-primary inline-flex items-center gap-2">
                  <Save className="w-4 h-4" />
          ...
          {active === 'security' && (
            <div className="card p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    className="input-field"
                    value={security.min_length ?? 10}
                    onChange={(e) => setSecurity((p: any) => ({ ...p, min_length: Number(e.target.value) }))}
                  />
                </div>
                <div className="flex items-end">
                  <label className="inline-flex items-center gap-2 text-sm text-slate-200">
                  ...
                  <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        type="number"
                        className="input-field"
                        value={security.rotation_days_default ?? 90}
                        onChange={(e) => setSecurity((p: any) => ({ ...p, rotation_days_default: Number(e.target.value) }))}
                      />
              ...
              <div className="flex justify-end">
                <button disabled={securitySaving || !canSettingsEdit} onClick={saveSecurity} className="btn-primary inline-flex items-center gap-2">
                  <Save className="w-4 h-4" />
          ...
          {active === 'plans' && (
            <div className="card p-6 space-y-6">
              <div>
                ...
                <p className="text-sm text-slate-400">Catálogo maestro en <code>cerebro.subscription_plans</code>. El guardado ya no envía <code>billing_model</code> ni <code>unit_price</code>.</p>
              </div>

              <div className="rounded-2xl border border-[rgba(148,163,184,0.12)] bg-white/5 p-4 space-y-4">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                  <div>
                    <div className="text-sm font-semibold text-slate-100">{planEditingCode ? `Editando: ${planEditingCode}` : 'Nuevo plan'}</div>
                    <div className="text-xs text-slate-400">Campos alineados al modelo vigente: code, name, description, price, price_model, is_active.</div>
                  </div>
                  <button type="button" onClick={resetPlanEditor} className="btn-secondary inline-flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="basico"
                      disabled={!canEditPlans || !!planEditingCode}
                    />
                    <p className="mt-1 text-xs text-slate-500">Se recomienda minúsculas para coincidir con tenants.plan.</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <select
                      className="input-field"
                      value={planForm.price_model}
                      onChange={(e) => setPlanForm((p) => ({ ...p, price_model: normalizePlanPriceModel(e.target.value) }))}
                      disabled={!canEditPlans}
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setPlanForm((p) => ({ ...p, is_active: e.target.value === 'true' }))}
                      disabled={!canEditPlans}
                    >
              ...
              <div className="flex justify-end gap-3">
                {planEditingCode && (
                  <button onClick={resetPlanEditor} className="btn-secondary inline-flex items-center gap-2">
                    Cancelar edición
                  </button>
                )}
                <button onClick={savePlan} disabled={!canEditPlans} className="btn-primary inline-flex items-center gap-2">
                  <Save className="w-4 h-4" />
                  {planEditingCode ? 'Actualizar plan' : 'Guardar plan'}
                </button>
              </div>
              ...
              <div className="border-t border-[rgba(148,163,184,0.12)] pt-4">
                ...
                <div className="mt-3">
                  {plansLoading ? (
                        ...
                        <tbody>
                          {plans.map((pl) => (
                            <tr key={pl.code} className="border-t border-[rgba(148,163,184,0.10)]">
                              ...
                              <td className="text-slate-400">{planPriceModelLabel(pl.price_model)}</td>
                              <td className="text-slate-400">{pl.is_active ? 'Activo' : 'Inactivo'}</td>
                    ...
                    <div className="text-sm text-slate-400">Sin planes. Revisa migraciones, permisos o el schema cerebro.</div>
                  )}
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    value={messageDraft.priority}
                    onChange={(e) => setMessageDraft((p: any) => ({ ...p, priority: e.target.value }))}
                  >
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setMessageDraft((p: any) => ({ ...p, body: e.target.value }))}
                  />
                </div>
                <div className="md:col-span-2">
      ...
      <div className="text-xs text-slate-500">Si ves errores de "tabla no existe" o "schema cache": ejecuta <b>supabase/migrations</b> y revisa RLS.</div>
    </div>
  )
}
