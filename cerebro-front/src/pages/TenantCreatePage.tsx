 ...
 * CEREBRO v4.9.0 — Correcciones:
 *  - Carga de planes desde public.subscription_plans (sin .schema('cerebro'))
 *    → La Edge Function valida contra public.subscription_plans; el frontend
 *      debe consultar el mismo catálogo.
 *  - Envío del campo `status` al payload de la Edge Function.
 *  - Mejor extracción de errores de la Edge Function (soporta body ya consumido).
 *  - Show/hide password para la clave temporal.
 */
...
/**
 * Extrae el mensaje de error de la respuesta de la Edge Function.
 * El SDK de Supabase puede pasar el error de distintas formas dependiendo
 * del código de estado y la versión del cliente.
 */
async function extractFunctionError(error: unknown): Promise<string> {
  ...
  try {
    ...
    // Caso 1: error tiene contexto con respuesta HTTP cloneable
    ...
    if (response?.clone) {
      ...
      if (clone?.text) {
        try {
          ...
          if (text?.trim()) return text.slice(0, 300).trim()
        } catch {
          // no-op
        }
      }
    ...
    // Caso 2: error directo con message
...
export default function TenantCreatePage() {
  ...
  // ── Carga de planes ─────────────────────────────────────────────────────────
  // CORRECCIÓN: La Edge Function valida plan_type contra public.subscription_plans.
  // El frontend debe consultar el mismo schema (public, no cerebro).
  useEffect(() => {
    let active = true
    ...
    const loadPlans = async () => {
      ...
      try {
        // public.subscription_plans — mismo origen que valida la Edge Function
        ...
        if (!mapped.length) {
          setPlans([])
          setPlansError('No existen planes activos en public.subscription_plans.')
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        console.error('Error en Edge Function admin-create-tenant:', error)
        toast.error(message)
        return
      }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 space-y-6">
          ...
          {/* ── Biométricos y política comercial ──────────────────────────── */}
          <div className="card p-6">
            <div className="flex items-center gap-2 mb-4">
